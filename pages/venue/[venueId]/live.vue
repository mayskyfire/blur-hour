<template>
  <div class="min-h-screen p-4">
    ...
    <div class="mb-6">
      ...
      <p class="text-slate-400">{{ filteredProfiles.length }} คน</p>
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-4">
      ...
      >
        ...
        <div class="text-center">
          <div class="w-16 h-16 bg-gradient-to-br from-neonCyan to-neonPink rounded-full mx-auto mb-2 flex items-center justify-center text-2xl">
        ...
        <div class="flex justify-center">
          <span :class="getActivityStatusColor(profile.activityStatus)" class="text-xs px-2 py-1 rounded-full bg-slate-800">
            {{ getActivityStatusEmoji(profile.activityStatus) }} {{ profile.activityStatus }}
          </span>
        </div>

        <!-- Mood & Zone -->
        <div class="text-center space-y-1">
          <p class="text-sm">{{ getMoodEmoji(profile.mood) }} {{ profile.mood }}</p>
        ...
        <!-- Social Media -->
        <div v-if="profile.lineId || profile.instagram || profile.tiktok" class="flex gap-2 justify-center">
          <a v-if="profile.lineId" :href="`https://line.me/ti/p/~${profile.lineId}`" target="_blank" class="text-green-400 text-lg hover:scale-110 transition-transform">📱</a>
          <a v-if="profile.instagram" :href="`https://instagram.com/${profile.instagram.replace('@', '')}`" target="_blank" class="text-pink-400 text-lg hover:scale-110 transition-transform">📸</a>
          <a v-if="profile.tiktok" :href="`https://tiktok.com/@${profile.tiktok.replace('@', '')}`" target="_blank" class="text-red-400 text-lg hover:scale-110 transition-transform">🎵</a>
        </div>
        ...
        <div class="flex gap-2 justify-center">
          <button
            ...
            class="p-2 bg-gradient-to-r from-slate-700 to-slate-600 rounded-lg hover:from-neonCyan/20 hover:to-neonPink/20 transition-all disabled:opacity-50"
          >
            <span class="text-lg">{{ vibe.emoji }}</span>
          </button>
          
          <!-- Report Button -->
          <button
            @click="openSafetyModal(profile.userId)"
            class="p-2 bg-slate-700 rounded-lg hover:bg-red-600/20 transition-all text-slate-400 hover:text-red-400"
          >
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-gradient-to-br from-neonCyan/20 to-neonPink/20 rounded-card border border-neonCyan p-6 text-center space-y-4 max-w-sm">
          <div class="text-5xl">{{ vibeResult.matched ? '🎉' : '✨' }}</div>
          <h2 class="text-xl font-bold">
            {{ vibeResult.matched ? 'Match แล้ว!' : 'ส่ง Vibe แล้ว!' }}
          </h2>
          <p class="text-slate-300">
            {{ vibeResult.matched ? 'คุณสองคนส่ง vibe ให้กัน' : 'รอดูว่าเขาจะตอบกลับไหม' }}
          </p>
          <button
...
const filteredProfiles = computed(() => {
  return profiles.value.filter(profile => {
    if (filters.gender.length && !filters.gender.includes(profile.gender)) return false
    if (filters.mood.length && !filters.mood.includes(profile.mood)) return false
    if (filters.zone.length && !filters.zone.includes(profile.zone)) return false
    return true
  })
})
...
const handleSendVibe = async (toUserId: string, vibeType: 'cheers' | 'music' | 'chat') => {
  sendingVibe.value = true
  try {
    const result = await sendVibe(toUserId, venueId, vibeType)
    vibeResult.value = result
...
const handleUserReported = () => {
  profiles.value = profiles.value.filter(p => p.userId !== selectedUserId.value)
}

onMounted(async () => {
  const currentProfile = await getCurrentProfile()
  if (!currentProfile || currentProfile.venueId !== venueId) {
